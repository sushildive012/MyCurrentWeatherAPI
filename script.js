...
async function getGeoLocation(inputString) {
    ...
    const url = `https://api.openweathermap.org/geo/1.0/direct?q=${encodeURIComponent(updatedLocationString)}&limit=${5}&appid=${apiKey}`;
    ...
    try {
        ...
        if (res.status === 401) {
            displayError(
                "Invalid or missing API key. Please check your apiKey variable."
            );
            return null;
        }
...
function displayWeather(foreCastData) {
    ...
    console.log(
        `[UI Rendered] Weather loaded for ${name}: ${temp}°C, ${weatherObject.main}`
    );
}
...
form.addEventListener("submit", async (e) => {
    ...
    if (length < 4) {
        // display error
        displayError(
            "To be precise try to enter city names having letters not less than 4"
        );
        return;
    }
...
Please choose the proper subscription https://openweathermap.org/price"
} 
